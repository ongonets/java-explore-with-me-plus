...
import ru.practicum.ewm.event.dto.RequestCountDto;
...
public interface RequestRepository extends JpaRepository<Request, Long> {
    ...
    @Query("SELECT new ru.practicum.ewm.event.dto.RequestCountDto(r.event.id, count(r.id)) " +
            "FROM Request r WHERE r.event IN :events " +
            "AND r.status = ru.practicum.ewm.event.model.RequestStatus.CONFIRMED " +
            "GROUP BY r.event.id")
